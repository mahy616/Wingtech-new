...
{
	qDebug() << "CImageCapture::SetCameraStatus bOpen" << bOpen;
	m_Mutex.lock();
	m_bOpen = bOpen;
	m_Mutex.unlock();
}
...
void CImageCapture::run()
{
	...
	{/////////////////////////////////////////////////////////////////////////////////
		...
		if (bStop)
		{
			qDebug() << "CImageCapture Stop";
			break;
		}
		//bOpen = true;
		//ResetGrabIndex = true;
		if (bOpen && ResetGrabIndex)
		{

			int nRet = m_MvCamera.GetOneFrameTimeout(m_pGrabBuf, nDataSize, &stImageInfo, 1000);
			if (nRet == MV_OK)
			{
				qDebug() << "ImageCaPture:" << m_GrabIndex << endl;
				Mat img = Convert2Mat(&stImageInfo, m_pGrabBuf);
				if (img.data == NULL)
				{
					continue;
				}
				Mat curImage = img.clone();// .clone();

				if (m_SystemType == CAMEREA_TEST)
				{
				
					emit SendCameraImage(curImage, m_Type, m, m_TestGrabIndex);
			
					if (m_TestGrabIndex <= 27)
					{
						m_TestGrabIndex++;
					}
					else
				...
				if (bStart)
				{
					if (m_Type == CAMERA_FIRST)
						emit SendImageToAlgo(curImage, m_Type, m_GrabIndex);
