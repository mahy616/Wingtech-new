...
#ifndef __WENTAI_CE_MIAN_MODULE_H__
#define __WENTAI_CE_MIAN_MODULE_H__

#include "ISegmentationModule.h"

namespace smartmore {

struct WenTaiCeMianResponse : SegmentationResponse {
    std::map<int, size_t> nums;
};

class DLL_API WenTaiCeMianModule : public ISegmentationModule {
   public:
    WenTaiCeMianModule();
    ~WenTaiCeMianModule();
    ...
    /**
            ...
            @return 返回码 ：0：成功； 其他值：失败
        */
    ResultCode Run(const SegmentationRequest &req, WenTaiCeMianResponse &rsp);
